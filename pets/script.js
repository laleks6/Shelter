import data from "./data__pets.js";
    ...
    if (quantity === 5) {
      if (count < 6 || count > 1) {
        for (let i = 0; i < arrMatrix[count].length; i++) {
          ...
          puppePersonImg[i].src = data[+arrMatrix[count][i]]["img"];
          namePuppe[i].textContent = data[+arrMatrix[count][i]]["name"];
        }
...
const normalWithScreen = window.matchMedia(
  "(min-width: 626px) and (max-width: 768px)"
);
...
const popUp = () => {
  ...
  const creatPopUp = () => {
    console.log(id);
    imgPopUp.src = data[id]["img"];
    namePopUp.textContent = data[id]["name"];
    breedPopUp.textContent = `${data[id]["type"]} - ${data[id]["breed"]}`;
    infoPopUp.textContent = data[id]["description"];

    strong[0].textContent = "Age: ";
    strong[1].textContent = "Inoculations: ";
    strong[2].textContent = "Diseases: ";
    strong[3].textContent = "Parasites: ";
    info__span[0].textContent = data[id]["age"];
    info__span[1].textContent = data[id]["inoculations"];
    info__span[2].textContent = data[id]["diseases"];
    info__span[3].textContent = data[id]["parasites"];
  };
...
popUp();
console.log(
  "Доделаю в течении 2 дней, пишите в дискорд (@laleks#0503), либо в проверочном чате RS"
);
